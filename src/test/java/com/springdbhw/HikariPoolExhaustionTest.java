...
@SpringBootTest
@Testcontainers
class HikariPoolExhaustionTest {
    ...
    static void registerDatabaseProperties(DynamicPropertyRegistry registry) {
        ...
        registry.add("spring.datasource.primary.maximum-pool-size", () -> 3);
    }
    ...
    // 4. Not enough Hikari Pool connections
    @Test
    void testHikariPoolExhaustionForNewRequests() {
        List<Connection> activeConnections = new ArrayList<>();

        try {
            for (int i = 0; i < 3; i++) {
                activeConnections.add(primaryDataSource.getConnection());
            }

            assertThrows(SQLTransientConnectionException.class, () -> primaryDataSource.getConnection());
